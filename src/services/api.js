...
export async function fetchBestSellers() {
  const response = await request('/products/best-sellers');
  return (response.data || []).map(mapProduct);
}

export async function fetchRecentReviews() {
  const response = await request('/products/reviews/recent');
  return response.data || [];
}

export async function fetchProductReviews(slug) {
  const response = await request(`/products/${slug}/reviews`);
  return response.data || {product: null, reviews: []};
}
